...
export function openLightbox(src, caption = '') {
  ...
  if (!_lightboxEl) {
    ...
    _lightboxEl.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.9);z-index:9000;display:flex;align-items:center;justify-content:center;';
    _lightboxImg = document.createElement('img');
    _lightboxImg.style.cssText = 'max-width:90vw;max-height:90vh;object-fit:contain;';
    _lightboxCaption = document.createElement('div');
    _lightboxCaption.style.cssText = 'position:absolute;bottom:40px;color:white;font-size:0.85rem;text-align:center;width:100%;';
    const closeBtn = document.createElement('button');
    closeBtn.textContent = '\u00D7';
    closeBtn.style.cssText = 'position:absolute;top:20px;right:20px;background:rgba(255,255,255,0.2);border:none;color:white;font-size:1.5rem;cursor:pointer;width:40px;height:40px;border-radius:50%;';
    closeBtn.addEventListener('click', closeLightbox);
    _lightboxEl.appendChild(_lightboxImg);
    _lightboxEl.appendChild(_lightboxCaption);
    _lightboxEl.appendChild(closeBtn);
    _lightboxEl.addEventListener('click', (e) => { if (e.target === _lightboxEl) closeLightbox(); });
