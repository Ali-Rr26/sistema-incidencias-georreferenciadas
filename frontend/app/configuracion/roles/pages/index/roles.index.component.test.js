/**
 * roles.index.component — table-actions migration tests.
 *
...
const FIXTURE_HTML = `
  <button id="btn-nuevo-rol"></button>
  ...
  <table><thead><tr><th></th><th></th><th></th></tr></thead><tbody id="tabla-body"></tbody></table>
...
const MOCK_ROLES = [
  { id: '1', name: 'Administrador', permissions_count: 12 },
  { id: '2', name: 'Operador', permissions_count: 5 },
];
...
beforeEach(async () => {
  ...
  const permsToReturn = new Set(['roles.update', 'roles.delete']);
  ...
  http.get.mockImplementation((path) => {
    if (path.startsWith('/roles')) return Promise.resolve({ data: MOCK_ROLES, meta: { total: 2 } });
  ...
  if (!componentModule) {
    componentModule = await import('./roles.index.component.js');
  }
...
describe('Desktop — permission-driven action rendering', () => {
  ...
  it('renders only Ver + Editar when user lacks delete permission', async () => {
    await renderIndexWithPermissions(new Set(['roles.update']));
  ...
  it('renders only Ver + Eliminar when user lacks update permission', async () => {
    await renderIndexWithPermissions(new Set(['roles.delete']));
  ...
  it('renders kebab disabled when user has no action permissions', async () => {
    await renderIndexWithPermissions(new Set(['roles.view']));
...
describe('Action handlers — CustomEvent delegation', () => {
  ...
  it('clicking Eliminar opens the Bootstrap Delete Modal', async () => {
    ...
    expect(document.getElementById('modal-eliminar-nombre').textContent).toBe('Administrador');
  });
});
