...
import { initSelect, getSelect, clearSelect, destroyAll } from '../../../../shared/select-search.js';

const LOCATION_LABELS = { country: 'País', province: 'Provincia', city: 'Ciudad' };

export default defineComponent({
    templateUrl: 'app/configuracion/organizaciones/pages/form/organizaciones.form.component.html',
    ...
    async onInit() {
        ...
        document.getElementById('form-titulo').textContent = esEdicion ? 'Editar Organización' : 'Nueva Organización';
        document.getElementById('card-titulo').textContent = esEdicion ? 'Editar Organización' : 'Nueva Organización';
        document.getElementById('breadcrumb-actual').textContent = esEdicion ? 'Editar' : 'Crear';
        ...
        async function cargarPadres(exceptId = null) {
            try {
                ...
                mostrarToast('No se pudieron cargar las organizaciones padre.', 'warning');
            }
        }
        ...
        function poblarSelectNativo(selId, items, textoDefault) {
            ...
            sel.innerHTML = `<option value="">${textoDefault}</option>` + items.map(i =>
        ...
        function onPaisChange() {
            ...
            if (val) {
                ...
                poblarSelectNativo(provinciaSel, pais?.children ?? [], '-- Seleccione --');
        ...
        function onProvinciaChange() {
            ...
            clearSelect(ciudadSel);
            if (val) {
                const pais = findNodeById(locationTree, parseInt(document.getElementById(paisSel).value));
                const provincia = pais?.children?.find(c => c.id === parseInt(val));
                poblarSelectNativo(ciudadSel, provincia?.children ?? [], '-- Opcional --');
        ...
        function actualizarLocationId() {
            ...
            document.getElementById('org-location').value = ciudad || provincia || pais;
        }
        ...
        async function initCascadingLocation(valorSeleccionado = null) {
            ...
            if (valorSeleccionado) {
                const ancestors = findAncestors(locationTree, parseInt(valorSeleccionado));
                if (ancestors) {
        ...
        document.getElementById('form-org').addEventListener('submit', async function (e) {
            ...
            const categoryIds = catSelect
                ? catSelect.getValue().map(Number)
                : [];
            ...
            try {
                ...
                setTimeout(() => { router.navigate('/organizaciones'); }, 800);
            } catch (err) {
