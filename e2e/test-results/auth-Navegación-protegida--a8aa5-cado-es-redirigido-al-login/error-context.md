...
- Name: auth.spec.ts >> Navegación protegida >> usuario no autenticado es redirigido al login
- Location: auth.spec.ts:97:7
...
Call log:
  - navigating to "/dashboard.html", waiting until "load"
...
> 98  |     await page.goto('/dashboard.html');
  ...
  112 | 
  113 |     // Esperar a que se valide y redirija
  114 |     await page.waitForURL(/.*login/, { timeout: 10000 });
  115 |   });
  116 | });
  117 | 
```